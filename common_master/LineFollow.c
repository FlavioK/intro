...
static void StateMachine(void) {
	...
	case STATE_FOLLOW_SEGMENT:
		...
		break;
	case STATE_TURN:
		if (MAZE_IsSolved()) { /* if maze solved: go back to beginning */
			turn = MAZE_GetSolvedTurn(&solveNbr);

			if(((turn != TURN_LEFT180) && (turn !=TURN_RIGHT180))){ /* if we dont have to go forward, we dont :) */
			...
			if (MAZE_EvaluteTurn(&finished, lefthand) == ERR_FAILED) { /* if maze not solved yet: solve it :)*/
				SHELL_SendString("Maze Failed -> State Stop!!!\r\n");
	...
	case STATE_FINISHED:
		...
		SHELL_SendString("LINE: Returned to start\r\n");
		BUZ_Beep(400,200);
		LF_currState = STATE_STOP;
		returned = FALSE;
		MAZE_ClearSolution();
